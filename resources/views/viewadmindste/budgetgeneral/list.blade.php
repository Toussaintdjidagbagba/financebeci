                                ...
                                <table id="tech-companies-1" class="table table-small-font table-bordered table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td style="vertical-align:middle; text-align: center;">{{ App\Providers\InterfaceServiceProvider::LibSouscompte($serv->sc) }}</td>
                            ...
                            <div class="form-group">
                                <div class="form-line">
                                    <select type="text" id="nbg" name="nbg" class="form-control" placeholder="">
                                        ...
                                        @endphp 
                                        @foreach($lbg as $lb)
                                            <option value="{{$lb->id}}">{{$lb->description}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
...
<div class="modal fade" id="impant" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="{{ route('SIBGA') }}"  enctype="multipart/form-data">
                    <input type="hidden" id="_token" name="_token" value="{{ csrf_token() }}" />
                    <div class="row clearfix">
                        <div class="col-md-12">
                            <label for="dlesc">Télécharger le fichier exemplaire : <a href="{{ route('GLBFORBGA') }}"> Exemplaire_budget_anterieur.xlsx </a> </label>
                        </div>
                    </div>
                    <br><hr><br>
                    <div class="row clearfix">
...
<div class="modal fade" id="imp" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="{{ route('SLGI') }}"  enctype="multipart/form-data">
                    
                    <div class="row clearfix">
                        <div class="col-md-12">
                            <label for="dlesc">Télécharger le fichier exemplaire : <a href="{{ route('GLBFORBG') }}"> Exemplaire_budget.xlsx </a> </label>
                        </div>
                    </div>
                    <br><hr><br>
                    <div class="row clearfix">
...
<div class="modal fade" id="exp" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    ...
    <script type="text/javascript">
        ...
        function exportparam(){
            ...
            document.getElementById('exportbuget').href = "{{ route('EBG') }}?mode="+mode+"&an="+an;
        }
        ...
        async function validelb(){
                ...
                if(token == "" || aebdt == "" || mbg == "" || nbg == ""){
                    ...
                    // En cours d'envoie
                        try {
                            let response = await fetch("{{route('ABG')}}",
                            {
                                method: 'POST',
                                headers: { 
                            ...
                            if(response.status == 200)
                            {
                                ...
                                if(reponse.status == 0)
                                {
                                    ...
                                    setTimeout(function(){
                                        window.location.href = "{{route('GLBG')}}";
                                    }, 3000);
        ...
        async function getupdate(data, name, lib){
            ...
            document.getElementById('seembg').innerHTML = '<select type="text" id="mbg_u" name="mbg_u" class="form-control" placeholder=""> <option value="'+data.sc+'"> '+lib+' </option> @php $comptes = App\Providers\InterfaceServiceProvider::allsouscomptes(); @endphp  @foreach($comptes as $compte)<option value="{{$compte->id}}">{{$compte->compte}}</option>@endforeach</select>'; 
            document.getElementById('see_nature').innerHTML = '<select type="text" id="nbg_u" name="nbg_u" class="form-control" placeholder=""> <option value="'+data.nature+'"> '+name+' </option>  @php  $lbg = App\Providers\InterfaceServiceProvider::alllignebudgetaire(); @endphp  @foreach($lbg as $lb) <option value="{{$lb->id}}">{{$lb->description}}</option> @endforeach </select>';
        ...
        async function validedelete(){
                ...
                dat = {_token: token, id: iddelete,};
                    ...
                    // En cours d'envoie
                        try {
                            let response = await fetch("{{route('DBG')}}",
                            {
                                method: 'POST',
                                headers: {
                                                'Access-Control-Allow-Credentials': true,
