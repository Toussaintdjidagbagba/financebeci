                        ...
                        <div class="header">
                            <h2>
                                ...
                                <div class="row pull-right m-r-8" style="display: flex; justify-content: space-between;">
                                    ...
                                    {{-- <div class="col-3">
                                        <button type="button" title="Importer" class="btn bg-deep-orange waves-effect" data-color="deep-orange" data-toggle="modal" data-target="#imp"><i class="material-icons"> cloud_upload </i> </butt0on>
                                    </div>&nbsp;
                                    <div class="col-3">
                                        <a href="{{ route('GELB') }}">  <button type="button" title="Exporter"  class="btn bg-deep-orange waves-effect" data-color="deep-orange" data-toggle="modal" data-target="#exp"><i class="material-icons">picture_as_pdf</i> </butt0on></a>
                                    </div>&nbsp; --}}
                                </div>
                            </h2>
                        </div>
                        <div class="body">
                            <div class="table-responsive" data-pattern="priority-columns">
                                <table id="tech-companies-1" class="table table-small-font table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @forelse($list as $unite)
                                        <tr>
                                            <td style="vertical-align:middle; text-align: center;">{{ $unite->id }}</td>
                                            <td style="vertical-align:middle; text-align: center;">{{ $unite->libelle }}</td>
                                            <td style="vertical-align:middle; text-align: center;">
                                                @if(in_array("update_service", session("auto_action")))
                                                    <button onclick="getupdate({{$unite}})" type="button" title="Modifier" data-toggle="modal" data-target="#updateUnite" class="btn btn-primary btn-circle btn-xs  margin-bottom-10 waves-effect waves-light">
                                                        <i class="material-icons">system_update_alt</i> 
                                                    </button>
                                                @endif

                                                @if(in_array("delete_service", session("auto_action")))
                                                    <button onclick="getdelete({{$unite}})" type="button" title="Supprimer" data-toggle="modal" data-target="#delete" class="btn btn-danger btn-circle btn-xs  margin-bottom-10 waves-effect waves-light"><i class="material-icons">delete_sweep</i> 
                                                    </button>
                                                @endif
                                            </td>
                                        </tr>
    ...
    <script type="text/javascript">
        ...
        async function validelb(){
            ...
            if((token == "" || unitelibelle == "")) {
                ...
                // En cours d'envoie
                try {
                    let response = await fetch("{{route('SAUTE')}}",
                    {
                        method: 'POST',
                        headers: { 
                    ...
                    if(response.status == 200)
                    {
                        ...
                        if(reponse.status == 0)
                        {
                            ...
                            setTimeout(function(){
                                window.location.href = "{{route('GLUTE')}}";
                            }, 3000);
                        }else{
        ...
        async function validedelete(){
            ...
            // En cours d'envoie
            try {
                let response = await fetch("{{route('DLUTE')}}",
                {
                    method: 'POST',
                    headers: {
                                    'Access-Control-Allow-Credentials': true,
